...
Dockerfile analyzer. Missing inputs remain representable; extra arguments that
may name host files or secrets prevent a completeness claim, while static
labels are included in the recipe identity. Keep its owned controls and
build-only semantics aligned with deployment, and run freeze consumer tests
when changing it.
