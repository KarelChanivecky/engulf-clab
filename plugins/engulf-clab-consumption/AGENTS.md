...
- Preserve `N/A` propagation, inode and canonical image-ID deduplication,
  retained-image fallback, `DEPLOYED`/`STOPPED`/`RECLAIMED` state, lab-owner
  sharing semantics, and two-second polling. Keep Docker tags in the separate
  `image_ref` field.
