...
LOCAL_SOURCES = {
    ...
    "plugins/engulf-clab-containers-core/src/engulf_clab_containers_core/containers/ldap-389ds/README.md": "eclab-container-ldap-389ds.md",
    "plugins/engulf-clab-containers-core/src/engulf_clab_containers_core/containers/proxy-node/README.md": "eclab-container-proxy-node.md",
    "plugins/engulf-clab-containers-core/src/engulf_clab_containers_core/containers/ubuntu-firefox-gui/README.md": "eclab-container-ubuntu-firefox-gui.md",
...
def synchronize(repo: Path, engulf: Path, check_only: bool, require_engulf: bool) -> None:
    ...
    if engulf.is_dir():
        mappings.extend((engulf / source, destination) for source, destination in ENGULF_SOURCES.items())
    elif require_engulf:
...
def parser() -> argparse.ArgumentParser:
    ...
    result.add_argument("--repo", type=Path, default=Path(__file__).resolve().parent.parent)
    ...
    update.add_argument(
        "--engulf-dir",
        type=Path,
        default=Path(os.environ.get("ENGULF_DIR", Path(__file__).resolve().parent.parent.parent / "engulf")),
    )
    update.add_argument("--require-engulf", action="store_true")
