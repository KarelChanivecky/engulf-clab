...
SKILL_NAME = "develop-eclab-lab-static"
...
def bundled_skill() -> Traversable | Path:
    bundle = resources.files("develop_eclab_lab_static_skill").joinpath("bundle")
    if bundle.is_dir():
        return bundle
...
def parser() -> argparse.ArgumentParser:
    result = argparse.ArgumentParser(
        prog="develop-eclab-lab-static-install",
        description=__doc__,
        epilog=(
            "Defaults may also be set with DEVELOP_ECLAB_LAB_STATIC_SKILLS_DIR and "
            "DEVELOP_ECLAB_LAB_STATIC_BACKUP_DIR. Otherwise the skills directory is "
            "$CODEX_HOME/skills, falling back to ~/.codex/skills. Explicit "
            "options take precedence."
        ),
    )
    configured_skills = os.environ.get("DEVELOP_ECLAB_LAB_STATIC_SKILLS_DIR")
    ...
    default_backups = Path(
        os.environ.get(
            "DEVELOP_ECLAB_LAB_STATIC_BACKUP_DIR",
    ...
    try:
        ...
        if target.exists() or target.is_symlink():
            ...
            if (
                ...
                or "name: develop-eclab-lab-static"
                not in definition.read_text(encoding="utf-8")
            ):
