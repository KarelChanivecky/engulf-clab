...
- Seed the `eclab` root pipeline, resolve only the one pipeline requested by the
  running executable, and compile its root-first inheritance chain in one pass.
  Do not compile a parent bundle and merge its generated output into a child.
- Pipeline declarations have one parent. Missing parents, cycles, conflicting
  declarations, and duplicate provider IDs in a resolved chain are fatal;
  declaration failures outside that chain are irrelevant.
...
- Keep artifacts and `latest.json` partitioned below
  `pipelines/<pipeline-id>/`; legacy unscoped cache records must not satisfy a
  request. Source resolution and the global lease remain shared.
- Retain only the latest generated fingerprint directory per pipeline after
  publishing `latest.json`. Prune only cache-owned fingerprint names; leave
  unknown entries, symlinks, and staging paths untouched, and never make a
  successful publish fail because stale-artifact cleanup could not complete.
...
```bash
.venv/bin/python -m compileall -q plugins/engulf-clab-schema/src
.venv/bin/python -m pytest -q plugins/engulf-clab-schema/tests
make check-skill
```
