...
```bash
.venv/bin/python -m compileall -q plugins/engulf-clab-ensure-vrnetlab/src
.venv/bin/python -m pytest -q plugins/engulf-clab-ensure-vrnetlab/tests
make check-skill
```

Checkout-helper changes require both ensure-plugin suites, plus the
`engulf-clab-vrnetlab-build` suite that consumes the prepared checkout.
