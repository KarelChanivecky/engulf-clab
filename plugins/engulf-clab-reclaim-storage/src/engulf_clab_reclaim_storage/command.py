...
from .model import Container, LabUse, ReclaimStoragePlan


class ReclaimStorageError(RuntimeError):
    pass
...
def plan(
    ...
    *,
    ...
    docker: DockerClient,
    program: str = "eclab reclaim-storage",
    ...
    if options.all:
        ...
        try:
            ...
            raise ReclaimStorageError(str(error)) from error
        universe = tuple(
            lab
            for lab in universe
...
def _selected_lab(
    ...
    if not isinstance(name, str) or not name:
        raise ReclaimStorageError(f"topology has no nonempty name: {topology}")
    matching = tuple(
        item
