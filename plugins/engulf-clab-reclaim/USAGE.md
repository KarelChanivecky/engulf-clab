...
reclamation aborts without deleting anything. Container and image deletion is
then best effort: every requested object is attempted, failures are logged, and
the command exits nonzero if anything could not be removed. Successfully
removed objects are not recreated when a later deletion fails.
